...
const char *host = "http://40.114.104.17/examples/api.php";
    ...
    Serial.println(root["data"][1].as<char*>());
      ...
      if(result_0["mode"]== "manual" && result_1["mode"]== "manual"){
          ...
          if (result_0["status"]== "on"){
             ...
             http.begin("http://40.114.104.17/examples/api_aktual_manual.php?status=nyala");
             int httpCode = http.GET();  
          }else{
            //Serial.println(F("MATIKAN stopkontak 2"));
            digitalWrite(relay,LOW);
            digitalWrite(LED2,HIGH);
            http.begin("http://40.114.104.17/examples/api_aktual_manual.php?status=mati");
            int httpCode = http.GET(); 
          }
          ...
          if (result_1["status"]== "on"){
            ...
            http.begin("http://40.114.104.17/examples/api_aktual_manual2.php?status=nyala");
            int httpCode = http.GET();  
          }else{
            //Serial.println(F("MATIKAN Stopkontak 2"));
            digitalWrite(LED2,LOW);
           //digitalWrite(relay2,LOW);
            http.begin("http://40.114.104.17/examples/api_aktual_manual2.php?status=mati");
            int httpCode = http.GET(); 
          }
      }
      ...
      if(result_0["mode"]== "auto" && result_1["mode"]== "auto"){
        ...
        if(timeClient.getMinutes() >= result_0["menit_hidup"] && timeClient.getMinutes() < result_0["menit_mati"]){
          if(timeClient.getSeconds() >= result_0["detik_hidup"]){
            digitalWrite(LED2, LOW);
            digitalWrite(relay,HIGH);
            http.begin("http://40.114.104.17/examples/proses_update_auto.php?status=on");
            http.begin("http://40.114.104.17/examples/api_aktual_manual.php?status=nyala");
            int httpCode = http.GET();
          }
        }else{
          if(timeClient.getSeconds() >= result_0["detik_mati"]){
            digitalWrite(LED2, HIGH);
            digitalWrite(relay,LOW);
            http.begin("http://40.114.104.17/examples/proses_update_auto.php?status=off");
            http.begin("http://40.114.104.17/examples/api_aktual_manual.php?status=mati");
            int httpCode = http.GET();
          }
        } 
        ...
        if(timeClient.getMinutes() >= result_1["menit_hidup"] && timeClient.getMinutes() < result_1["menit_mati"]){
          if(timeClient.getSeconds() >= result_1["detik_hidup"]){
            digitalWrite(LED2, HIGH);
            http.begin("http://40.114.104.17/examples/proses_update_auto2.php?status=on");
            http.begin("http://40.114.104.17/examples/api_aktual_manual2.php?status=nyala");
            int httpCode = http.GET();
          }
        }else{
          if(timeClient.getSeconds() >= result_1["detik_mati"]){
            digitalWrite(LED2, LOW);
            http.begin("http://40.114.104.17/examples/proses_update_auto2.php?status=off");
            http.begin("http://40.114.104.17/examples/api_aktual_manual2.php?status=mati");
            int httpCode = http.GET();
          }
        }
